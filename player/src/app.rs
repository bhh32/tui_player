...
use core::{
    MediaInfo, MediaPlayer, MediaSourceType, YouTubeConfig, YouTubePlayer, create_media_player,
    detect_media_type, render::RenderConfig,
};
...
impl App {
    ...
    pub fn open_media(&mut self, path: &str) -> Result<()> {
        ...
        match media_type {
            ...
            MediaSourceType::YouTube => {
                self.set_status(
                    format!("Opening YouTube: {} (this may take a moment...)", path),
                    Color::Yellow,
                );
            }
            _ => {}
        ...
        let player_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            match media_type {
                MediaSourceType::LocalFile => create_media_player(path, Some(render_config)),
                MediaSourceType::YouTube => {
            ...
            Ok(result) => match result {
                Ok(player) => {
                    ...
                    match media_type {
                        ...
                        MediaSourceType::YouTube => {
                            self.set_status(
                                format!("Opened YouTube video: {}", path),
                                Color::Green,
                            );
                        }
                        _ => {}
    ...
    pub fn navigate_to(&mut self, path: impl AsRef<Path>) -> Result<()> {
        ...
        if path.is_dir() {
            ...
            self.file_browser
                .history
                .push(self.file_browser.current_dir.clone());
    ...
    fn handle_file_browser_key(&mut self, key: KeyEvent) -> Result<()> {
        match key.code {
            ...
            KeyCode::PageDown => {
                if let Some(selected) = self.file_browser.selected {
                    let new_selected =
                        (selected + 10).min(self.file_browser.files.len().saturating_sub(1));
                    self.file_browser.selected = Some(new_selected);
                }
            }
            KeyCode::Home => {
    ...
    fn handle_player_mouse(&mut self, mouse: MouseEvent, area: Rect) -> Result<()> {
        ...
        let action = if let (Some(player), Some(media_info)) = (&mut self.player, &self.media_info)
        {
            ...
            // Check if we're clicking on the play/pause button
                ...
                if mouse.row >= button_y
                    && mouse.row < button_y + button_height
                    && mouse.column >= button_x - (button_width / 2)
                    && mouse.column < button_x + (button_width / 2)
                {
                    if let MouseEventKind::Down(MouseButton::Left) = mouse.kind {
                        player.toggle_pause();
                ...
                // Check if we're clicking on the back button
                        ...
                        && mouse.column < back_button_x + 3
                    {
                        if let MouseEventKind::Down(MouseButton::Left) = mouse.kind {
                            ...
                            Some(PlayerAction::Seek(
                                new_pos,
                                format!("Back 10s to {:.1}s", new_pos),
                            ))
                        } else {
                            None
                        }
                    ...
                    // Check if we're clicking on the forward button
                        ...
                        if mouse.row == button_y
                            && mouse.column >= forward_button_x - 3
                            && mouse.column < forward_button_x + 3
                        {
                            if let MouseEventKind::Down(MouseButton::Left) = mouse.kind {
                                let new_pos =
                                    match (player.get_position() + 10.0, media_info.duration) {
                                ...
                                Some(PlayerAction::Seek(
                                    new_pos,
                                    format!("Forward 10s to {:.1}s", new_pos),
                                ))
                            } else {
                                None
                            }
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        if self.view == AppView::YoutubeSearch
            && self.youtube_search.results.is_empty()
            && !self.youtube_search.searching
        {
